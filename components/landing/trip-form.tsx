...
export function TripForm() {
  ...
  const createAndNavigate = async (data: TripFormValues, mode?: string) => {
    setIsLoading(true);
    ...
    try {
      ...
      const path = mode ? `/plan/${itinerary.id}?mode=${mode}` : `/plan/${itinerary.id}`;
      router.push(path);
    } catch (error) {
  ...
  const onSubmit = (data: TripFormValues) => createAndNavigate(data);
  const onSubmitPerfect = (data: TripFormValues) => createAndNavigate(data, "perfect");
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5" noValidate>
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                className="flex-1"
                disabled={isLoading}
                onClick={form.handleSubmit(onSubmitPerfect)}
              >
                <span className="flex items-center gap-2">
                  ✨ {t("perfectArrangeButton")}
                </span>
              </Button>
            </div>
          </form>
