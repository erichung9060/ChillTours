...
**跨日 closing time 修正（`safeClose`）**：若 `opening_hours.close` 為 `"00:00"` 或小於 `open`，視為跨日午夜，換算為 `86400s`（`24:00`），避免 Vroom invalid window 錯誤。
...
```
有 meal type    → 用餐時段前完全不選；在時段內考慮餐廳自身 opening_hours
有 opening_hours → 等到開門才服務（close 為 00:00 視為跨日）
...
## 用餐佔位符（Meal Placeholder）

用餐佔位符是「完美安排」路線優化的中間步驟，在真實餐廳確定前佔住用餐位置。

### 觸發條件

`optimizeDayWithMealSearch`（store.ts）在以下條件下自動插入佔位符：

| 條件                 | 說明                                                       |
|----------------------|------------------------------------------------------------|
| 行程時間涵蓋用餐時段 | 例如 09:00–21:00 包含午餐（11:00–14:00）→ 插入午餐佔位符 |
| 當天已有對應 type    | 已有 `type: "lunch"` → 不重複插入                          |

### 佔位符行為

- `isMealPlaceholder: true` — 距離矩陣該行/列全設為 `0`，不影響其他景點間的路線成本
- 完整優化（`optimizeRouteFull`）跳過佔位符的 Places 豐富化
- 排序結果中，佔位符的前後鄰居位置即為推薦餐廳的搜尋中心點

### Phase B 流程

```
optimizeDayWithMealSearch(dayNumber)
  ├─ 插入用餐佔位符（lunch/dinner）
  ├─ POST /api/optimize-route（含佔位符，distance = 0）
  ├─ 解析佔位符在排序中的前後景點 → 計算搜尋中心點
  ├─ POST /api/places-nearby（Google Places Nearby Search）
  └─ 更新 mealSuggestions state → 顯示 MealSuggestionPanel

使用者選擇餐廳後 selectMealRestaurant(dayNumber, placeholderId, restaurant)
  ├─ 以真實餐廳活動取代佔位符，寫入行程
  └─ 觸發 optimizeDayFull（含真實餐廳）第二次完整優化
```
