...
describe("DaySchema - start_time / end_time", () => {
  ...
  it("缺少前導零的格式（8:00）應通過", () => {
    ...
    expect(() =>
      DaySchema.parse({ ...baseDay, start_time: "8:00" })
    ).not.toThrow();
  });

  it("小時超過 23 應拋錯", () => {
    expect(() =>
      DaySchema.parse({ ...baseDay, start_time: "25:00" })
    ).toThrow();
  });

  it("分鐘超過 59 應拋錯", () => {
    expect(() =>
      DaySchema.parse({ ...baseDay, end_time: "10:60" })
    ).toThrow();
  });

  it("格式錯誤（無冒號）應拋錯", () => {
    expect(() =>
      DaySchema.parse({ ...baseDay, start_time: "0800" })
    ).toThrow();
  });

  it("格式錯誤（文字）應拋錯", () => {
    expect(() =>
      DaySchema.parse({ ...baseDay, start_time: "morning" })
    ).toThrow();
  });
  ...
  it("只帶 start_time 不帶 end_time 也應通過", () => {
    expect(() =>
      DaySchema.parse({ ...baseDay, start_time: "08:00" })
    ).not.toThrow();
  });
});
