    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (itinerary) => {
          ...
          await expect(saveItinerary(itineraryToSave)).rejects.toThrow(FreeTierLimitError);
        }
      ),
      { numRuns: 100 }
    );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (nonExistentId) => {
          ...
          await expect(loadItinerary(nonExistentId)).rejects.toThrow(ItineraryNotFoundError);
        }
      ),
      { numRuns: 100 }
    );
...
describe('Property 23: Itinerary Save-Load Round-trip', () => {
  ...
  it('should preserve all data in save-load round-trip', async () => {
    await fc.assert(
      fc.asyncProperty(
        itineraryArbitrary,
        async (originalItinerary) => {
          const { id, created_at, updated_at, ...itineraryToSave } = originalItinerary;
          ...
          loaded.days.forEach((day, i) => {
            ...
            expect(day.activities.length).toBe(itineraryToSave.days[i].activities.length);
            ...
            day.activities.forEach((activity, j) => {
              ...
              expect(activity.duration_minutes).toBe(originalActivity.duration_minutes);
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (itineraryId) => {
          ...
          await expect(loadItinerary(itineraryId)).rejects.toThrow(ItineraryNotFoundError);
        }
      ),
      { numRuns: 100 }
    );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (nonExistentId) => {
          ...
          await expect(deleteItinerary(nonExistentId)).rejects.toThrow(ItineraryNotFoundError);
        }
      ),
      { numRuns: 100 }
    );
