    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (itinerary, sourceIdx, targetIdx) => {
          ...
          expect(itinerary.days[0].activities.map((a: Activity) => a.id)).toEqual(
            originalSourceIds,
          );
          expect(itinerary.days[1].activities.map((a: Activity) => a.id)).toEqual(
            originalTargetIds,
          );
        },
      ),
    ...
    await fc.assert(
      fc.asyncProperty(
        fc
          ...
          .map(
            ([id, user_id, title, destination, day1Activities, day2Activities, day3Activities]) => {
              const itinerary: Itinerary = {
                id,
                user_id,
                title,
        ...
        async (itinerary, sourceIdx, targetIdx) => {
          ...
          const originalUnaffectedActivities = unaffectedDay.activities.map((a: Activity) => ({
            ...a,
          }));
